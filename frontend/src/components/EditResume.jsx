...
const EditResume = () => {
  ...
  const [atsIsLoading, setAtsIsLoading] = useState(false)
  ...
  const generateResumeText = () => {
    ...
    const contact = [c.email, c.phone, c.location, c.linkedin, c.github, c.website].filter(Boolean).join(" | ")
    ...
    if (Array.isArray(resumeData.projects)) {
      lines.push("Projects:")
      resumeData.projects.forEach(pj => {
    ...
    if (Array.isArray(resumeData.certifications) && resumeData.certifications.length) {
      ...
      resumeData.certifications.forEach(cert => {
        const line = [cert.title, cert.issuer, cert.year].filter(Boolean).join(" - ")
        if (line) lines.push(line)
      })
    }

    if (Array.isArray(resumeData.languages)) {
      const langs = resumeData.languages.map(l => (typeof l === 'string' ? l : l.name)).filter(Boolean)
      if (langs.length) {
    ...
    if (Array.isArray(resumeData.interests) && resumeData.interests.length) {
      lines.push("Interests:")
      lines.push(resumeData.interests.filter(Boolean).join(", "))
    }
  ...
  const handleAnalyzeATS = async () => {
    try {
      ...
      if (!resumeText.trim()) {
        toast.error("Please add resume details before analyzing")
        return
      }
  ...
  return (
    <DashboardLayout>
      <div className={containerStyles.main}>
        <div className={containerStyles.header}>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <Check size={16} />
              <span className="text-sm">Analyze ATS</span>
            </button>
          </div>
        </div>
      ...
      <Modal
        ...
        title="ATS Analyzer"
        showActionBtn
        actionBtnText={atsIsLoading ? "Analyzing..." : "Run Analysis"}
        actionBtnIcon={atsIsLoading ? (<Loader2 size={16} className="animate-spin" />) : (<Check size={16} className="text-white" />)}
        onActionClick={handleAnalyzeATS}
      >
        <div className="space-y-4 pt-6">
          <div className="text-sm text-gray-600">
            Improve accuracy by adding a target job description. We will compare your resume against it and generate an ATS score with suggestions.
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">Job Description (optional)</label>
            <textarea
              className="w-full min-h-32 rounded-lg border border-gray-300 bg-white/80 backdrop-blur p-3 text-sm placeholder-gray-400 outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500"
          ...
          {atsReport && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
              <h3 className="text-base font-semibold mb-2">ATS Report</h3>
              <div className="max-h-80 overflow-auto rounded-lg bg-gray-50 p-3">
                <pre className="whitespace-pre-wrap text-sm text-gray-800 leading-relaxed">{atsReport}</pre>
              </div>
            </div>
          )}
        </div>
