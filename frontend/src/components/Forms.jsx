...
export const ProfileInfoForm = ({ profileData, updateSection }) => {
  ...
  const generateProfessionalSummary = async () => {
    ...
    if (!fullName || !designation) {
      toast.error("Please fill in your name and designation first");
      return;
    }
    ...
    try {
      ...
      const prompt = `Write a professional resume summary of 60–70 words for the designation ${designation}. The summary should highlight relevant skills, years of experience, core strengths, and value the candidate can bring to the role. Use a formal and impactful tone suitable for ATS-friendly resumes. Do not include personal details or company names. Return ONLY the summary text, no analysis or additional comments.`;

      console.log('Sending AI request:', { prompt, designation });

      const { data } = await axiosInstance.post(API_PATHS.RESUME.ANALYZE_TEXT(), {
        resumeText: prompt,
        jobDescription: `Role: ${designation}`
      });

      console.log('AI response:', data);

      // Extract the generated summary from the AI response
      const generatedSummary = data?.analysis || '';
      
      // Clean up the response to get just the summary
      const cleanSummary = generatedSummary
        .replace(/^.*?(summary|profile|professional):/i, '')
        .replace(/^\s*[-•]\s*/, '')
        .replace(/^.*?(resume|analysis|report).*?$/gi, '')
      ...
      if (cleanSummary) {
        updateSection('summary', cleanSummary);
      ...
      toast.error("Failed to generate summary. Please check your OpenAI configuration.");
    } finally {
          ...
          <div className="md:col-span-2">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1 text-xs font-medium text-violet-600 bg-violet-50 rounded-lg hover:bg-violet-100 transition-colors disabled:opacity-50"
              >
                {isGeneratingSummary ? (
                  <>
            ...
            <p className="text-xs text-gray-500 mt-2">
              💡 Use the AI Generate button to create a professional summary based on your name and designation
            </p>
          </div>
        </div>
      </div>
